...
public class NetworkServer {
    ...
    private void handleMessages(DatagramPacket clientRequest) throws IOException, InterruptedException {
        ...
        boolean alreadyHasClientIP = false;
        for (DatagramPacket aClientIP : clientIP) {
            if (aClientIP.getAddress().equals(clientRequest.getAddress())) {
                alreadyHasClientIP = true;
                break;
            }
        }
        if (!alreadyHasClientIP) {
            System.out.println("adding new ip: " + clientRequest.getAddress());
